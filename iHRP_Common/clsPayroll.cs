...
//							+ "'");
						string Err = "";
						string SQL = "PR_spCalPayroll @Activity ='CalSalaryItemUser',@Month= '" + sMonth 
							+ "',@LSSalaryItemDataID= '"+sLSSalaryItemDataID+"',@Formula= '"+sFormula
...
//			}
		...
		public static DataTable GetDataSalaryExport(string sMonth, string LSCompanyID, string LSLevel1ID, string LSLevel2ID
			, string LSLevel3ID, string EmpID, string EmpName, string sLanguageID, string sAccountID, string sFormula)
		{
			try
			{
		...
		public static string sSendApproved(DataGrid dtgList)
		{
			string sErrMess="";
			SqlCommand  cmd = new SqlCommand();
			SqlConnection SQLconn= new SqlConnection(ConfigurationSettings.AppSettings["pstrConnectionString"]);
			...
			try
			{
				...
				cmd.Transaction= SQLconn.BeginTransaction();
				cmd.CommandType = CommandType.StoredProcedure;
				cmd.CommandText = "PR_spfrmApprovedParoll";
				...
				for (int i = 0;i<= dtgList.Items.Count - 1;i++)
				{
					...
					{
						cmd.Parameters.Clear();
						cmd.Parameters.Add("@Activity", SqlDbType.NVarChar, 30).Value = "SendAproved";					
						...
						cmd.Parameters.Add("@Month", SqlDbType.NVarChar,7).Value= dtgList.Items[i].Cells[1].Text.Trim(); 
						DropDownList cboNextUserID = (DropDownList)dtgList.Items[i].FindControl("cboNextUserID");
						string sNextUserID = cboNextUserID.SelectedValue;
						cmd.Parameters.Add("@NextUserID", SqlDbType.NVarChar,20).Value= sNextUserID; 
