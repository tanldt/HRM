 ...
 namespace iHRPCore.MdlHR
{
	...
	/// </summary>
	public class HR_frmOrganizationChart : System.Web.UI.UserControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			treeView.Load(Server.MapPath("GeneralInfo/XsltFile/treeview1.xslt"));
		...
		private XmlDocument MakeLevelsXML(string mEmpID)
		{
			try
			{
				...
				string mSql = "sp_HR_frmOrganizationChart 1";
				...
				for (int i=1; i<= CountLevel1; i++)
				{
					...
					objFrag = doc.CreateDocumentFragment();
					mSql = "sp_HR_frmOrganizationChart 2,@Parent='" + strID1 + "'";
				...
				for (int i=1; i<= CountLevel1; i++)
				{
					...
					for (int j=0; j<= CountLevel2; j++)
					{
						...
						objFrag = doc.CreateDocumentFragment();
						mSql = "sp_HR_frmOrganizationChart 3,@Parent='" + strID2 + "'";
						//mTable = HR_clsCommon.GetDataTable(mSql);
						mTable = clsCommon.GetDataTable(mSql);
						if (mTable.Rows.Count > 0)
						{
				...
				for (int i=1; i<= CountLevel1; i++)
				{
					...
					for (int j=0; j<= CountLevel2; j++)
					{
						...
						for (int k=0; k<= CountLevel3; k++)
						{
							...
							objFrag = doc.CreateDocumentFragment();
							mSql = "sp_HR_frmOrganizationChart 4,@Parent='" + strID4 + "',@Parent1='" + strID3 + "'";
							//mTable = HR_clsCommon.GetDataTable(mSql);
							mTable = clsCommon.GetDataTable(mSql);
							if (mTable.Rows.Count > 0)
							{
