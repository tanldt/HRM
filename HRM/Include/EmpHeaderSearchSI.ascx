...
<%@ Control Language="c#" AutoEventWireup="false" Codebehind="EmpHeaderSearchSI.ascx.cs" Inherits="iHRPCore.Include.EmpHeaderSearchSI" TargetSchema="http://schemas.microsoft.com/intellisense/ie5" %>
<%
...
<script language="javascript">	
	...
	{
		var all = document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel2").length;
		var all1 = document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel3").length;
		var all2 = document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel1").length;
		for(i=0; i<all2; i++)
		{	
			document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel1").remove(0);			
		};
		
		for(i=0; i<all; i++)
		{	
			document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel2").remove(0);			
		};
		for(i=0; i<all1; i++)
		{	
			document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel3").remove(0);			
		};
		var strCompany = document.getElementById("_ctl0:EmpHeaderSearchSI1:cboCompany").value; 
		document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel1").add(new Option('',''));
		for(i=0; i<<%=Total1%>;i++)
		{
			if (DS1[0][i]==strCompany)
			{
				document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel1").add(new Option(DS1[2][i],DS1[1][i]));
			};
		};		
		all = document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel1").length;
		document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel1").selectedIndex = 0;
		ChangeLevel1();			
	}
	...
	function ChangeLevel1()
	{
		...
		var strLevel1ID = document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel1").value; 
		document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel2").add(new Option('',''));
		
		for(i=0; i<<%=Total2%>;i++)
		{
			if (DS2[0][i]==strLevel1ID)
			{
				document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel2").add(new Option(DS2[2][i],DS2[1][i]));
			};
		};		
		all = document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel2").length;
		document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel2").selectedIndex = 0;
		document.getElementById("_ctl0:EmpHeaderSearchSI1:txtLevel1ID").value = strLevel1ID;
		ChangeLevel2();			
	}
	
	function ChangeLevel2()
	{
		var all = document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel3").length;
		
		for(i=0; i<all; i++)
		{	
			document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel3").remove(0);			
		};
		var strLevel2ID = document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel2").value; 
		document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel3").add(new Option('',''));
		
		for(i=0; i<<%=Total3%>;i++)
		{
			if (DS3[0][i]==strLevel2ID||strLevel1ID=="")
			{
				document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel3").add(new Option(DS3[2][i],DS3[1][i]));
			};
		};		
		document.getElementById("_ctl0:EmpHeaderSearchSI1:txtLevel2ID").value = document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel2").value;
		ChangeLevel3();	
	}
	function ChangeLevel3()
	{	
		document.getElementById("_ctl0:EmpHeaderSearchSI1:txtLevel3ID").value = document.getElementById("_ctl0:EmpHeaderSearchSI1:cboLevel3").value;
	}
</script>
<!--table-->
		...
		<TD background="images/table_13.jpg" align="center">
			...
			<TABLE id="Table1" cellSpacing="0" cellPadding="1" width="100%" border="0">
				<TR>
					...
					<TD width="15%"><asp:textbox id="txtEmpID" onkeypress="checkKey('_ctl0_btnSearch')" CssClass="input" runat="server"
					...
					<TD width="27%"><asp:textbox id="txtEmpName" onkeypress="checkKey('_ctl0_btnSearch')" CssClass="input" runat="server"
							Width="95%"></asp:textbox><%if(Request.Params["EmpID"] == null && Request.Params["EmpName"] == null){%><%}%></TD>
				...
				<TR>
					<TD>
						<asp:label id="Label11" runat="server" CssClass="label">Bệnh viện</asp:label></TD>
					<TD colSpan="5">
						<asp:dropdownlist onkeypress="checkKey('_ctl0_btnSearch')" id="cboHospital" runat="server" CssClass="combo"
							Width="100%"></asp:dropdownlist></TD>
				</TR>
				<TR style="DISPLAY: none">
					<TD></TD>
			...
			<script language="javascript">
//	document.getElementById("_ctl0:EmpHeaderSearchSI1:txtEmpID").focus();
			</script>
			<!--End table body-->
		</TD>
...
<script>
	/*function ChangeLanguage()
	{
		...
		{
			document.getElementById("_ctl0_EmpHeaderSearchSI1_optStatus_0").innerHTML = "Active"; 
			document.getElementById("_ctl0_EmpHeaderSearchSI1_optStatus_1").innerHTML = "Termination"; 
			document.getElementById("_ctl0_EmpHeaderSearchSI1_optStatus_2").innerHTML = "All"; 
		}
		else
		{
			document.getElementById("_ctl0_EmpHeaderSearchSI1_optStatus_0").value = "Hi?n di?n"; 
			document.getElementById("_ctl0_EmpHeaderSearchSI1_optStatus_1").value = "Ngh? vi?c"; 
			document.getElementById("_ctl0_EmpHeaderSearchSI1_optStatus_2").value = "T?t c?"; 
		}
	}
	ChangeLanguage();*/
</script>
