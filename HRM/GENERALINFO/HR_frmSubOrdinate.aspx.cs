...
{
	/// <summary>
	/// Summary description for HR_frmSubOrdinate.
	/// </summary>
	public class HR_frmSubOrdinate : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			if (Convert.ToInt16(Session["CheckAccountLogin"]) == 1)
			{
				if (!Page.IsPostBack)
				{
					DataTable mTable = HR_clsCommon.GetDataTable("Select EMPLOYEEID from UMS_tblUserAccount where UserGroupID=N'" + Session["CurUser"].ToString() + "'");
					XmlDocument docLevels = MakeLevelsXML(mTable.Rows[0][0].ToString());
