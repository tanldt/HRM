...
namespace iHRPCore.Reports.v11
{
	/// <summary>
	...
	public class clsBaocaoExcel
	{
		...
		public string HTMLDataTable(DataTable oDataTable)
		{
			...
			foreach(DataRow oDataRow in oDataTable.Rows)	
			{
				...
				foreach(DataColumn oDataColumn in oDataTable.Columns)	
				{
					strHTML +=  ("<TD class=xl60>&nbsp;" + oDataRow[oDataColumn.ColumnName] + "</TD>");
				}
				strHTML +=  ("</TR>");
			}
			strHTML +=  ("</table>");
		...
		#endregion
		#region FillColorTable
		public string FillColor(DataTable WorkDayTable,string strBody)
		{		
			foreach(DataRow oDataRow in WorkDayTable.Rows)	
			{
				string strdd = oDataRow["DateID"].ToString();
				if (oDataRow["TypeDate"].ToString() == "3")	
				{
					int idd = strBody.IndexOf("$D"+strdd+"$");
					string aaa = strBody.Substring(idd-35,40);
					if(idd > -1)
						strBody = strBody.Replace(strBody.Substring(idd-35,40),"class=xl85 style='border-top:none'>$D" + strdd + "$");
				}
				else
				{
					int idd = strBody.IndexOf("$D"+strdd+"$");
					if(idd > -1)
						strBody = strBody.Replace(strBody.Substring(idd-35,40),"class=xl86 style='border-top:none'>$D" + strdd + "$");
				}
			}
			return strBody;
		}
		...
		public string strReportBasic_FillColorDay(System.Data.DataTable oDataTable,System.Data.DataTable WorkDayTable)
		{
			string strReturn = "";
			string strHeader = "";
			string strBody = "";
			...
			//strHeader = strHeader.Replace("$Title$",strTitle);
			strHeader = HeaderReplace(strHeader);
			strBody = FillColor(WorkDayTable,strBody);
			strBody = AddDataHTMLCapHeader(oDataTable,strBody);
			strBottom = FooterReplace(strBottom);
