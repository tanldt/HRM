namespace iHRPCore.MdlPR
{
	...
	/// </summary>
	public class UnfrequentIncomeList : System.Web.UI.UserControl
	{
        ...
        protected System.Web.UI.WebControls.Label lblUNType;
        protected System.Web.UI.WebControls.Label lblPRMonth;
        ...
        , string strFromDate, string strToDate,string strUnfrequentIncomeTypeID,string strMMYYY,string strLanguage,System.Web.UI.Page pPage)
        ...
        {            
            ...
            try
            {
                string strSql = "PR_spfrmUNFREQUENTINCOME @Activity='GetDataListSearch',@EmpID=N'" + strEmpID + "', @EmpName=N'" + strEmpName
                    + "',@LSCompanyID='" + strCompanyID + "', @LSLevel1ID='" + strLevel1 + "', @LSLevel2ID='" + strLevel2 + "',@LSLevel3ID='" + strLevel3
                    + "',@FromDate=N'" + strFromDate.Trim() + "',@ToDate=N'" + strToDate + "',@UnfrequentIncomeTypeID=N'" + strUnfrequentIncomeTypeID + "',@Month=N'" + strMMYYY + "',@UserGroupID='" + sAccountLogin + "',@LanguageID='" + strLanguage + "'";
                if (strStatus != "")
		...
		private void LoadDataResult()
		{
			...
			try
			{
				...
				dt =GetListSearch(strEmpID, strEmpName,strCompanyID, strLevel1, strLevel2, strLevel3,strStatus,txtFromDate.Text,txtToDate.Text
					,cboUnfrequentIncomeTypeID.SelectedValue.Trim(),txtMonth.Text.Trim(),strLanguage, this.Page);
...
//				grdData.DataBind();
		...
		private DataTable Read_Data()
		{
			...
			dtb = GetListSearch(strEmpID, strEmpName,strCompanyID, strLevel1, strLevel2, strLevel3,strStatus,txtFromDate.Text,txtToDate.Text
				,cboUnfrequentIncomeTypeID.SelectedValue.Trim(),txtMonth.Text.Trim(),strLanguage, this.Page);
			return dtb;
		}
        ...
        private void grdData_ItemCommand_1(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
        {
            ...
            {
                Session["ssUnfrequentIncomeID"] = grdData.Items[e.Item.ItemIndex].Cells[0].Text.Trim();
                Session["EmpID"] = grdData.Items[e.Item.ItemIndex].Cells[1].Text.Trim();
                Session["ssStatusBonus"]="Edit";
                Response.Redirect("Editpage.aspx?ModuleID=PR&ParentID=68&FunctionID=533&Ascx=MdlPR/unfrequentincome.ascx");
            }
        }
