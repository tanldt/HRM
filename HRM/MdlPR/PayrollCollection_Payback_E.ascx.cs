namespace iHRPCore.MdlPR
{
	...
	/// <summary>
	///		Summary description for PayrollCollection_Expat.
	/// </summary>
	public class PayrollCollection_Payback_E : System.Web.UI.UserControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!Page.IsPostBack)
			{
				...
				DataRow drData= clsPRCollection.LoadDatePR2_Payback();
				if (drData!=null)
				{
		...
		private void BindDataGrid()
		{
			...
			try
			{
				dtList = clsPRCollection.GetDataSalary_Payback_HLHV2(cboCompanyID.SelectedValue,cboLevel1ID.SelectedValue,cboLevel2ID.SelectedValue,cboLevel3ID.SelectedValue,txtEmpID.Text.Trim(),txtEmpName.Text.Trim(),txtMonth.Text.Trim());
		...
		{			
			lblErr.Text=string.Format(clsPRCollection.CheckIsPR2_Payback(strLanguage),txtSalPeriod.Text,txtMonth.Text);
		...
		private void btnNewPayroll_Click(object sender, System.EventArgs e)
		{
			...
			try
			{		
				strReturn = clsPRCreateNewPayroll.CreatePayroll2_Payback(txtFromDate.Text, txtToDate.Text,txtMonth.Text,txtMonth.Text,txtSalPeriod.Text);
				if(strReturn.Trim()!="")
				{
					lblErr.Text = "";
					clsChangeLang.popupWindow(this.Parent,"CP_0001_P",strLanguage,"",0);

				}
				else
				{
					clsChangeLang.popupWindow(this.Parent,"CP_0002_P",strLanguage,"",1);
				}
			}
			catch
			{
		...
		private void btnCalulate_Click(object sender, System.EventArgs e)
		{
			...
			string strErr=clsPRCollection.CalculateSalary_Payback2("",sFromDate,sToDate,cboCompanyID.SelectedValue,cboLevel1ID.SelectedValue,cboLevel2ID.SelectedValue,cboLevel3ID.SelectedValue,strLanguage);
		...
		private void btnDelete_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				clsCommon.DeleteListRecord("PR_spPayrollCollection2_Payback","EmpID",SqlDbType.NVarChar,12,strID);				
		...
		{
			clsChangeLang.popupWindow(this.Parent,string.Format(clsPRCollection.LockPR2_Payback(strLanguage),txtSalPeriod.Text,txtMonth.Text),"",1);
			DataRow drData= clsPRCollection.LoadDatePR2_Payback();
			if (drData!=null)
			{
				txtMonth.Text=drData["Month"].ToString();
			...
			//cangtt -- 09052006 -- kiểm tra bảng lương có tồn tại hay chưa
			checkIsPR();
			//-------------------------------------------------------------			
			lblErr.Text="";
		}
