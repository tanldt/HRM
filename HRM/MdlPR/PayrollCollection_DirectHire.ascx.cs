namespace iHRPCore.MdlPR
{
	...
	public class PayrollCollection_DirectHire : System.Web.UI.UserControl
	{
		...
		{			
			...
			if (!Page.IsPostBack)
			{
				...
				DataRow drData= clsPRCollection.LoadDatePR1();
				if (drData!=null)
				{
		...
		private void BindDataGrid()
		{
			...
			try
			{
				dtList = clsPRCollection.GetDataSalary_HLHV1(cboCompanyID.SelectedValue,cboLevel1ID.SelectedValue,cboLevel2ID.SelectedValue,cboLevel3ID.SelectedValue,txtEmpID.Text.Trim(),txtEmpName.Text.Trim(),txtMonth.Text.Trim());
		...
		{			
			lblErr.Text=string.Format(clsPRCollection.CheckIsPR1(strLanguage),txtSalPeriod.Text,txtMonth.Text);
		...
		private void btnNewPayroll_Click(object sender, System.EventArgs e)
		{
			...
			try
			{		
				strReturn = clsPRCreateNewPayroll.CreatePayroll1(txtFromDate.Text, txtToDate.Text,txtMonth.Text,txtMonth.Text,txtSalPeriod.Text);
				if(strReturn.Trim()!="")
				{
					lblErr.Text = "";
		...
		private void btnCalulate_Click(object sender, System.EventArgs e)
		{
			...
			string strErr=clsPRCollection.CalculateSalary1(sFromDate,sToDate,cboCompanyID.SelectedValue,cboLevel1ID.SelectedValue,cboLevel2ID.SelectedValue,cboLevel3ID.SelectedValue,strLanguage);
		...
		private void btnDelete_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				clsCommon.DeleteListRecord("PR_spPayrollCollection1","EmpID",SqlDbType.NVarChar,12,strID);				
		...
		{
			clsChangeLang.popupWindow(this.Parent,string.Format(clsPRCollection.LockPR1(strLanguage),txtSalPeriod.Text,txtMonth.Text),"",1);
			DataRow drData= clsPRCollection.LoadDatePR1();
			if (drData!=null)
			{
				txtMonth.Text=drData["Month"].ToString();
