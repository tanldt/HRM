namespace iHRPCore.MdlPR
{
	...
	/// </summary>
	public class PayrollCollection_Payback_S: System.Web.UI.UserControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!Page.IsPostBack)
			{
				...
				DataRow drData= clsPRCollection.LoadDatePR3_Payback();
				if (drData!=null)
				{
		...
		private void BindDataGrid()
		{
			...
			try
			{
				dtList = clsPRCollection.GetDataSalary_Payback_HLHV3(cboCompanyID.SelectedValue,cboLevel1ID.SelectedValue,cboLevel2ID.SelectedValue,cboLevel3ID.SelectedValue,txtEmpID.Text.Trim(),txtEmpName.Text.Trim(),txtMonth.Text.Trim());
		...
		{			
			lblErr.Text=string.Format(clsPRCollection.CheckIsPR3_Payback(strLanguage),txtSalPeriod.Text,txtMonth.Text);
		...
		private void btnNewPayroll_Click(object sender, System.EventArgs e)
		{
			...
			try
			{		
				strReturn = clsPRCreateNewPayroll.CreatePayroll3_Payback(txtFromDate.Text, txtToDate.Text,txtMonth.Text,txtMonth.Text,txtSalPeriod.Text);
				if(strReturn.Trim()!="")
				{
					lblErr.Text = "";
		...
		private void btnCalulate_Click(object sender, System.EventArgs e)
		{
			...
			string strErr=clsPRCollection.CalculateSalary_Payback3("",sFromDate,sToDate,cboCompanyID.SelectedValue,cboLevel1ID.SelectedValue,cboLevel2ID.SelectedValue,cboLevel3ID.SelectedValue,strLanguage);
		...
		private void btnDelete_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				clsCommon.DeleteListRecord("PR_spPayrollCollection3_Payback","EmpID",SqlDbType.NVarChar,12,strID);				
		...
		{
			clsChangeLang.popupWindow(this.Parent,string.Format(clsPRCollection.LockPR3_Payback(strLanguage),txtSalPeriod.Text,txtMonth.Text),"",1);
			DataRow drData= clsPRCollection.LoadDatePR3_Payback();
			if (drData!=null)
			{
				txtMonth.Text=drData["Month"].ToString();
