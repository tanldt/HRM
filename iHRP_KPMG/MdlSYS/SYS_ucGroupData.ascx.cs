namespace MdlSYS
{
	...
	/// <summary>
	///		Summary description for SYS_ucGroupData.
	/// </summary>
	public class SYS_ucGroupData : System.Web.UI.UserControl
	{
		...
		private bool CheckValidData()
		{
			...
			try
			{
				...
				{
					dtb = clsCommon.GetDataTable("UMS_sptblUserAccount 'GetByIDData', @result='',@UserGroupID='" + this.txtGroupID.Text.Trim() + "'");
					if (dtb.Rows.Count > 0)
					{
						//this.lblErr.Text = "This group ID has already existed!";
						clsChangeLang.popupWindow(this.Parent,"ucG0004",strLanguage,"",0);
						return false;
					}
				}
				return true;
		...
		private void FillData(DataRow row)
		{
			try
			{
				...
				txtGroupID.CssClass="inputReadOnly";
		...
		private void BindDataGrid()
		{
			...
			try
			{
				dtb = clsCommon.GetDataTable("UMS_sptblUserAccount 'GetAllGroupData', @result=''");
				/*
		...
		private void btnSave_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if (this.txtGroupID.ReadOnly)
					param.Value = "UpdateData";
				else
					param.Value = "AddNewData";
				...
				param.Value = 1;
				param = cmd.Parameters.Add("@Description",SqlDbType.NVarChar,255);
				param.Value = this.txtDescription.Text.Trim();
				cmd.ExecuteNonQuery();
					...
					if (chkSelect.Checked)
					{
						try
						{
							...
							if (strresult == "-3" || strresult == "-4")
							{
								...
								clsChangeLang.popupWindow(this.Parent,"ucG0005",strLanguage,"",0);
								return;
							}
						}
						catch(Exception exp)
		...
		private void dgGroups_ItemCommand(object source, DataGridCommandEventArgs e)
		{
			if (e.CommandName == "hpLink")
			{
				...
				DataTable dtb = clsCommon.GetDataTable("UMS_sptblUserAccount 'GetByIDData', @result='',@UserGroupID='" + strGroupID + "'");
				if (dtb.Rows.Count > 0)
